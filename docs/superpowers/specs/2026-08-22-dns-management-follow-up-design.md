...
`dns/zones.nix` will declare every ordinary A, CNAME, MX, and TXT record in the reviewed Cloudflare exports dated 2026-08-22. Cloudflare-maintained SOA and apex NS records stay omitted. All exported TTL values of `1` are represented as `ttl = "auto"`; numeric TTL values remain numeric. Names are relative to their zone, CNAME and MX targets retain their trailing dots, and A/CNAME proxy state is copied from the export.
...
- Renderer fixture assertions verify TTLs and CNAME target output in addition to the existing record shape checks.
